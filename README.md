...
**About the country list:** Country lists are never entirely neutral. WorldHunt uses the 193 UN member states as its starting point and deliberately includes Palestine, Vatican City, and Western Sahara, for a total of 196 playable countries. Kosovo, Taiwan, dependencies, and other non-catalog territories are not playable answers.

Just type a country and press `Enter`. Canonical English names and documented aliases are accepted; matching normalizes case, accents, whitespace, and punctuation but does not make fuzzy guesses. Suggestions appear after two characters if you want a hand with the name. Invalid and repeated guesses do not consume an attempt.
...
| `+`, arrows | Zoom in and pan the map |
| `-` with an empty country field, `_`, or mouse wheel | Zoom out |
...
WorldHunt defaults to truecolor. Set `WORLDHUNT_COLOR=truecolor`, `WORLDHUNT_COLOR=ansi256`, or `WORLDHUNT_COLOR=mono` to choose a palette. `NO_COLOR` selects monochrome when no explicit mode is set. Distance and border clues remain textual in every mode.

## Requirements

- Rust 1.88 or newer. Install it with [Rustup](https://rustup.rs/).
